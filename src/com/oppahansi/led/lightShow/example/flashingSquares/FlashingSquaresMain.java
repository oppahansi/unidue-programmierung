...
package com.oppahansi.led.lightShow.example.flashingSquares;

import java.util.Random;

import ledControl.BoardController;

public class FlashingSquaresMain {
	
	static BoardController controller;
	static int[][][] colors;
	static int[] sizes = new int[]{6,4,3,2,1};
	static int[][] squareColors	= new int[][]{{0, 0, 127},
											  {0, 127, 0},
											  {0, 127, 127},
											  {127, 0, 0},
											  {127, 0, 127},
	...
	public static void main(String[] args) {
		...
		while (true){
			for (int i = 0; i < 10; i++){
				...
				controller.updateBoard();								// Update des Bilds
			}
		}
		...
		for (int x = 0; x < 12; x++){
			for (int y = 0; y < 12; y++){
				for (int color = 0; color < 3; color++){
					FlashingSquaresMain.colors[x][y][color] = (((x/size) + (y/size) + inverse) % 2 == 0) ? 127 : colors[color]; 
				}
			}
		}
	}
